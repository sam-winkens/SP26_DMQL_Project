...
CREATE TABLE Victim_Type ( --Stores demographic information for victim
	Victim_ID INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, --we need this because victim id is not provided in data
	Vict_Age INT,
	Vict_Sex VARCHAR(1),
...
CREATE TABLE Occurence( -- Holds the date and time combinations for crime occurences
	Occurence_ID BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, --we are not given occurrence_id in data
...
CREATE TABLE Crime_Report ( --This table is for every unique crime reported in the table. holds almost all the information at the table
	DR_NO BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, --this is optional because from our test data we are given primary ids to work with however, we auto generate the id for the user so they don't have to do it themselves.
